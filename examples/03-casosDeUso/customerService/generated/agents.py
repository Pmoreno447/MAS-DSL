...
from prompt import VALIDATORPROMPT, INFOEXTRACTORPROMPT, ANSWERWRITERPROMPT, SHIPPINGHELPERPROMPT, PAYMENTHELPERPROMPT, PRODUCTHELPERPROMPT, GENERALHELPERPROMPT, PROBLEMORCHESTRATORPROMPT
...
async def nodeValidator(state: State):
    ...
    messages = (
        [SystemMessage(content=VALIDATORPROMPT)]
        + state["messages"]
...
async def nodeInfoExtractor(state: State):
    ...
    messages = (
        [SystemMessage(content=INFOEXTRACTORPROMPT)]
        + state["messages"]
...
def nodeAnswerWriterNode(state: State):
    """Agente encargado de responder al usuario"""
    result = modelAnswerWriterNode.invoke(
        [SystemMessage(content=ANSWERWRITERPROMPT)]
        + state["messages"]
...
async def nodeShippingHelper(state: State):
    ...
    messages = (
        [SystemMessage(content=SHIPPINGHELPERPROMPT)]
        + state["messages"]
        + [HumanMessage(content=f"""
...
async def nodePaymentHelper(state: State):
    ...
    messages = (
        [SystemMessage(content=PAYMENTHELPERPROMPT)]
        + state["messages"]
        + [HumanMessage(content=f"""
            orderId: {state.get("orderId", "No registrado aún")}
            issueDescription: {state.get("issueDescription", "No registrado aún")}
...
async def nodeProductHelper(state: State):
    """Especialista en problemas de envios."""
    messages = (
        [SystemMessage(content=PRODUCTHELPERPROMPT)]
        + state["messages"]
        + [HumanMessage(content=f"""
            orderId: {state.get("orderId", "No registrado aún")}
            issueDescription: {state.get("issueDescription", "No registrado aún")}
...
async def nodeGeneralHelper(state: State):
    ...
    messages = (
        [SystemMessage(content=GENERALHELPERPROMPT)]
        + state["messages"]
        + [HumanMessage(content=f"""
            orderId: {state.get("orderId", "No registrado aún")}
            issueDescription: {state.get("issueDescription", "No registrado aún")}
