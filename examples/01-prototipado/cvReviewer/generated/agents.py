...
from prompt import EXTRACTORPROFILE, EVALUATORPROFILE, REPORTGENERATORPROFILE, NOTIFIERPROFILE
...
def nodeExtractor(state: State):
    ...
    result = modelExtractor.invoke(
        [SystemMessage(content=_system_prompt(EXTRACTORPROFILE, state))]
        + state["messages"]
...
def nodeEvaluator(state: State):
    ...
    result = modelEvaluator.invoke(
        [SystemMessage(content=_system_prompt(EVALUATORPROFILE, state))]
        + state["messages"]
...
def nodeReportGenerator(state: State):
    ...
    result = modelReportGenerator.invoke(
        [SystemMessage(content=_system_prompt(REPORTGENERATORPROFILE, state))]
        + state["messages"]
        + [HumanMessage(content=f"""
            candidateName: {state.get("candidateName", "No registrado aún")}
            candidateEmail: {state.get("candidateEmail", "No registrado aún")}
...
async def nodeNotifier(state: State):
    """"""
    messages = (
        [SystemMessage(content=_system_prompt(NOTIFIERPROFILE, state))]
        + state["messages"]
        + [HumanMessage(content=f"""
            candidateName: {state.get("candidateName", "No registrado aún")}
            candidateEmail: {state.get("candidateEmail", "No registrado aún")}
