...
from prompt import RESEARCHERPROFILE, WRITERPROFILE, CRITICPROFILE, EDITORPROFILE
...
def nodeResearcher(state: State):
    ...
    result = modelResearcher.invoke(
        [SystemMessage(content=RESEARCHERPROFILE)]
        + state["messages"]
...
def nodeWriter(state: State):
    ...
    result = modelWriter.invoke(
        [SystemMessage(content=WRITERPROFILE)]
        + state["messages"]
        + [HumanMessage(content=f"""
            topic: {state.get("topic", "No registrado aún")}
...
def nodeCritic(state: State):
    ...
    result = modelCritic.invoke(
        [SystemMessage(content=CRITICPROFILE)]
        + state["messages"]
        + [HumanMessage(content=f"""
...
def nodeEditor(state: State):
    ...
    result = modelEditor.invoke(
        [SystemMessage(content=EDITORPROFILE)]
        + state["messages"]
        + [HumanMessage(content=f"""
            draft: {state.get("draft", "No registrado aún")}
